...
<aside class="sb">
  ...
  <div class="sb-sec">Terrain</div>
  <div class="sb-it" onclick="gp('signal',this)"><span class="sb-ic">🔴</span>Signalements<span class="sb-new" id="sb-sig" style="display:none">!</span></div>
  <div class="sb-it" onclick="gp('events',this)"><span class="sb-ic">🎪</span>Événements</div>
...
<div class="page" id="p-signal">
  <div class="ph">
    <div class="ph-icon" style="background:#fee2e2">🔴</div>
    <div><div class="ph-title">Signalements &amp; incidents</div><div class="ph-sub">Main courante terrain — tout le monde voit tout</div></div>
    <div class="ph-act">
      <button class="btn btn-p btn-sm" onclick="om('signal')">+ Signaler</button>
    </div>
  </div>
  <div class="scr" style="padding:0">
    <!-- KPI signalements -->
    <div style="display:flex;gap:10px;padding:.85rem 1.4rem;background:#fff;border-bottom:1px solid var(--w2);flex-wrap:wrap">
      <div class="kpi" style="flex:1;min-width:100px"><div class="kpi-v" id="sk-tot" style="color:var(--ink)">0</div><div class="kpi-l">Total</div></div>
      <div class="kpi" style="flex:1;min-width:100px"><div class="kpi-v" id="sk-new" style="color:var(--red)">0</div><div class="kpi-l">Nouveaux</div></div>
      <div class="kpi" style="flex:1;min-width:100px"><div class="kpi-v" id="sk-ec" style="color:var(--amber)">0</div><div class="kpi-l">En cours</div></div>
      <div class="kpi" style="flex:1;min-width:100px"><div class="kpi-v" id="sk-re" style="color:var(--g4)">0</div><div class="kpi-l">Résolus</div></div>
    </div>
    <!-- Filtres -->
    <div class="fb">
      <select class="fsel" id="sf-type" onchange="fSignal()"><option value="">Tous types</option>
      ...
      <select class="fsel" id="sf-statut" onchange="fSignal()"><option value="">Tous statuts</option>
        <option>Nouveau</option><option>En cours</option><option>Résolu</option><option>Non retenu</option>
      </select>
      <input class="fsrch" id="sf-q" placeholder="🔍  Lieu, description…" oninput="fSignal()">
      <span class="fcnt" id="sf-cnt"></span>
    </div>
    <div id="signal-list" style="padding:1rem 1.4rem"></div>
  </div>
</div>
...
<div class="page" id="p-events">
  <div class="ph">
    <div class="ph-icon" style="background:#fef3c7">🎪</div>
    <div><div class="ph-title">Événements à Vizille</div><div class="ph-sub">Agenda des manifestations locales — public et municipal</div></div>
    <div class="ph-act">
      <button class="btn btn-p btn-sm" onclick="om('event')">+ Ajouter</button>
    </div>
  </div>
  <div class="scr">
...
<div class="ov" id="ov-signal">
  <div class="modal">
    <div class="mhd"><h3>🔴 Nouveau signalement</h3><button class="mclose" onclick="cm()">×</button></div>
    <div class="ff"><label>Titre / Objet *</label><input class="fi" id="sig-ti" placeholder="Ex: Lampadaire éteint, Nid de poule, Graffiti…"></div>
    <div class="fr2">
      <div class="ff"><label>Type</label>
        <select class="fi" id="sig-ty">
          <option>Voirie</option><option>Éclairage</option><option>Propreté</option>
          <option>Espace vert</option><option>Sécurité</option><option>Bâtiment</option><option>Autre</option>
        </select>
      </div>
      <div class="ff"><label>Urgence</label>
        <select class="fi" id="sig-urg">
    ...
    <div class="ff"><label>Lieu précis *</label><input class="fi" id="sig-lieu" placeholder="Rue, quartier, point de repère…"></div>
    <div class="ff"><label>Description</label><textarea class="fi" id="sig-desc" placeholder="Décrivez le problème constaté…"></textarea></div>
    <div class="fr2">
      <div class="ff"><label>Service concerné</label>
        <select class="fi" id="sig-serv">
          <option value="">— Non attribué —</option>
          <option>Service Technique</option><option>Police Municipale</option>
          <option>CCAS</option><option>Service Culturel</option><option>Service Enfance</option><option>DGS</option>
        </select>
      </div>
      <div class="ff"><label>Signalé par</label><input class="fi" id="sig-par" placeholder="Votre nom"></div>
    </div>
    <div class="mft"><button class="btn btn-g" onclick="cm()">Annuler</button><button class="btn btn-p" onclick="svSignal()">Enregistrer</button></div>
  </div>
</div>

<div class="ov" id="ov-signal-detail">
  <div class="modal" style="width:min(640px,95vw)">
    <div class="mhd"><h3 id="sig-detail-title">Signalement</h3><button class="mclose" onclick="cm()">×</button></div>
    <div id="sig-detail-body"></div>
    <div class="mft">
      <button class="btn btn-g" onclick="cm()">Fermer</button>
      <select class="fi" id="sig-new-statut" style="width:auto;padding:7px 10px">
        <option value="">Changer le statut…</option>
        <option>En cours</option><option>Résolu</option><option>Non retenu</option>
      </select>
      <button class="btn btn-p" onclick="updateSignal()">Mettre à jour</button>
    </div>
  </div>
</div>

<div class="ov" id="ov-event">
  <div class="modal">
    <div class="mhd"><h3>🎪 Ajouter un événement</h3><button class="mclose" onclick="cm()">×</button></div>
    <div class="ff"><label>Titre *</label><input class="fi" id="ev-ti" placeholder="Ex: Marché du dimanche, Loto des pompiers, AG OMS…"></div>
    <div class="fr2">
    ...
    <div class="fr2">
      <div class="ff"><label>Lieu</label><input class="fi" id="ev-lieu" placeholder="Salle, place, parc…"></div>
      <div class="ff"><label>Type</label>
        <select class="fi" id="ev-ty">
    ...
    <div class="ff"><label>Description</label><textarea class="fi" id="ev-desc" placeholder="Informations complémentaires…"></textarea></div>
    <div class="ff"><label>Organisateur</label><input class="fi" id="ev-org" placeholder="Association, service municipal…"></div>
    <div class="mft"><button class="btn btn-g" onclick="cm()">Annuler</button><button class="btn btn-p" onclick="svEvent()">Enregistrer</button></div>
  </div>
</div>
...
var P=[], ST={}, AG=[], DC=[], NF=[], CHAT=[], ELUS=ELUS0.slice(), ANN=[], TASKS=[], SIGN=[], EVTS=[];
...
function init(){
  ...
  apiGet('/api/all').then(d=>{
    ...
    updSigBadge(); fSignal(); renderEvents();
...
/* ── ACCUEIL ────────────────────────────────────────────────────────── */
/* ── SIGNALEMENTS ───────────────────────────────────────────────────────────── */
var SIG_TYPES={'Voirie':'🛣','Eclairage':'💡','Proprete':'🗑','Espace vert':'🌿','Securite':'🛡','Batiment':'🏗','Autre':'📋'};
var SIG_COL={Nouveau:'var(--red)',En_cours:'var(--amber)',Resolu:'var(--g4)',Non_retenu:'var(--i4)'};
var _sigId=null;

function updSigBadge(){
  var nb=SIGN.filter(s=>s.statut==='Nouveau').length;
  var b=$('sb-sig');if(b)b.style.display=nb>0?'inline':'none';
  el('sk-tot',SIGN.length);
  el('sk-new',SIGN.filter(s=>s.statut==='Nouveau').length);
  el('sk-ec',SIGN.filter(s=>s.statut==='En cours').length);
  el('sk-re',SIGN.filter(s=>s.statut==='Résolu').length);
}

function fSignal(){
  var ty=v('sf-type'),st=v('sf-statut'),q=v('sf-q').toLowerCase();
  var r=SIGN.filter(s=>(!ty||s.type===ty)&&(!st||s.statut===st)&&(!q||(s.titre||'').toLowerCase().includes(q)||(s.lieu||'').toLowerCase().includes(q)||(s.description||'').toLowerCase().includes(q)));
  el('sf-cnt',r.length+' signalement(s)');
  var MOIS=['Jan','Fev','Mar','Avr','Mai','Jun','Jul','Aou','Sep','Oct','Nov','Dec'];
  var URG={'normale':'','importante':'🟡 ','urgente':'🔴 '};
  $('signal-list').innerHTML=r.length?r.map(s=>{
    var col=s.statut==='Nouveau'?'var(--red)':s.statut==='En cours'?'var(--amber)':s.statut==='Résolu'?'var(--g4)':'var(--i4)';
    var d=new Date(s.date||Date.now());
    return '<div style="background:#fff;border-radius:var(--R);border:1px solid var(--w2);padding:.9rem 1.1rem;margin-bottom:10px;box-shadow:var(--s1);cursor:pointer;transition:.15s" onclick="openSignal('+s.id+')" onmouseover="this.style.boxShadow=\'var(--s2)\'" onmouseout="this.style.boxShadow=\'var(--s1)\'">'
      +'<div style="display:flex;align-items:flex-start;gap:12px">'
      +'<div style="width:44px;height:44px;border-radius:10px;background:'+col+'22;border:1px solid '+col+'44;display:flex;align-items:center;justify-content:center;font-size:1.2rem;flex-shrink:0">'+(SIG_TYPES[s.type]||'📋')+'</div>'
      +'<div style="flex:1">'
      +'<div style="display:flex;align-items:center;gap:8px;margin-bottom:4px">'
      +'<span style="font-size:.84rem;font-weight:700;font-family:var(--fd)">'+(URG[s.urgence]||'')+s.titre+'</span>'
      +'<span style="font-size:.63rem;font-weight:700;padding:2px 7px;border-radius:8px;background:'+col+'22;color:'+col+'">'+s.statut+'</span>'
      +'<span style="font-size:.63rem;font-weight:600;padding:2px 7px;border-radius:8px;background:var(--w2);color:var(--i3)">'+s.type+'</span>'
      +'</div>'
      +'<div style="font-size:.74rem;color:var(--i3)">📍 '+s.lieu+(s.service?' — 🏢 '+s.service:'')+'</div>'
      +(s.description?'<div style="font-size:.71rem;color:var(--i4);margin-top:3px;line-height:1.4">'+s.description.substring(0,100)+(s.description.length>100?'…':'')+'</div>':'')
      +'<div style="font-size:.63rem;color:var(--i4);margin-top:4px;font-family:var(--fm)">Signalé par '+( s.signale_par||'—')+' · '+s.created+'</div>'
      +'</div>'
      +'<button class="btn btn-d btn-sm" style="flex-shrink:0" onclick="event.stopPropagation();delSignal('+s.id+')">×</button>'
      +'</div></div>';
  }).join(''):'<div class="empty" style="padding:3rem"><div class="empty-ico">✅</div><div class="empty-t">Aucun signalement</div><div class="empty-s">Cliquez sur "+ Signaler" pour enregistrer un incident.</div></div>';
}
...
function svSignal(){
  ...
  apiPost('/api/signalements',d).then(r=>{if(r.ok){SIGN.unshift(r.item);cm();fSignal();updSigBadge();toast('Signalement enregistré');
  ['sig-ti','sig-lieu','sig-desc','sig-par'].forEach(i=>{var e=$(i);if(e)e.value='';});}});
}

function openSignal(id){
  _sigId=id;
  var s=SIGN.find(x=>x.id===id);if(!s)return;
  var col=s.statut==='Nouveau'?'var(--red)':s.statut==='En cours'?'var(--amber)':s.statut==='Résolu'?'var(--g4)':'var(--i4)';
  $('sig-detail-title').textContent='🔴 '+s.titre;
  $('sig-detail-body').innerHTML='<div style="background:'+col+'15;border-radius:var(--r);padding:.85rem 1rem;margin-bottom:1rem;border-left:3px solid '+col+'">'
    +'<div style="font-size:.82rem;font-weight:700;margin-bottom:.25rem">'+s.titre+'</div>'
    +'<div style="font-size:.74rem;color:var(--i3)">📍 '+s.lieu+' · 🏷 '+s.type+' · ⚡ '+(s.urgence||'normale')+'</div>'
    +'</div>'
    ...
    +'<div style="font-size:.72rem;background:var(--warm);border-radius:var(--r);padding:.6rem .75rem"><span style="color:var(--i3);display:block;font-size:.62rem;font-weight:700;text-transform:uppercase;letter-spacing:.05em;margin-bottom:2px">Signalé par</span>'+(s.signale_par||'—')+'</div>'
    +'<div style="font-size:.72rem;background:var(--warm);border-radius:var(--r);padding:.6rem .75rem"><span style="color:var(--i3);display:block;font-size:.62rem;font-weight:700;text-transform:uppercase;letter-spacing:.05em;margin-bottom:2px">Service</span>'+(s.service||'Non attribué')+'</div>'
    +'<div style="font-size:.72rem;background:var(--warm);border-radius:var(--r);padding:.6rem .75rem"><span style="color:var(--i3);display:block;font-size:.62rem;font-weight:700;text-transform:uppercase;letter-spacing:.05em;margin-bottom:2px">Statut actuel</span><span style="color:'+col+';font-weight:700">'+s.statut+'</span></div>'
    +'<div style="font-size:.72rem;background:var(--warm);border-radius:var(--r);padding:.6rem .75rem"><span style="color:var(--i3);display:block;font-size:.62rem;font-weight:700;text-transform:uppercase;letter-spacing:.05em;margin-bottom:2px">Créé le</span>'+s.created+'</div>'
    +'</div>'
    +'<div style="font-size:.76rem;font-weight:700;color:var(--i2);margin-bottom:.5rem">Historique</div>'
    +'<div style="display:flex;flex-direction:column;gap:6px">'
    +((s.historique||[]).map(h=>'<div style="display:flex;align-items:center;gap:8px;font-size:.72rem;padding:.5rem .7rem;background:var(--warm);border-radius:var(--r)">'
      +'<span style="font-weight:700;color:var(--g3)">'+h.statut+'</span><span style="color:var(--i3)">—</span><span>'+h.auteur+'</span><span style="color:var(--i4);font-family:var(--fm);margin-left:auto">'+h.ts+'</span>'
      +(h.commentaire?'<br><span style="color:var(--i3)">'+h.commentaire+'</span>':'')
      +'</div>').join(''))
    +'</div>';
  $('sig-new-statut').value='';
  om('signal-detail');
}

function updateSignal(){
  var nst=v('sig-new-statut');if(!nst){toast('Choisissez un statut');return;}
  apiPut('/api/signalements/'+_sigId+'/statut',{statut:nst,auteur:ME.nom}).then(d=>{
    if(d.ok){
      SIGN=SIGN.map(s=>s.id===_sigId?Object.assign({},s,{statut:nst}):s);
      cm();fSignal();updSigBadge();toast('Statut mis à jour : '+nst);
    }
  });
}
...
function delSignal(id){
  ...
  apiDel('/api/signalements/'+id).then(d=>{if(d.ok){SIGN=SIGN.filter(s=>s.id!==id);fSignal();updSigBadge();toast('Supprimé');}});
}

/* ── ÉVÉNEMENTS ──────────────────────────────────────────────────────────────── */
var EV_TYPES={municipal:'🏛',associatif:'🤝',culturel:'🎭',sportif:'⚽',commemoration:'🎖',autre:'📌'};
var EV_COLORS={municipal:'var(--g3)',associatif:'var(--amber)',culturel:'#8B5CF6',sportif:'var(--blue)',commemoration:'#7f8c8d',autre:'var(--i3)'};

function renderEvents(){
  var now=new Date().toISOString().slice(0,10);
  var sorted=EVTS.slice().sort((a,b)=>a.date>b.date?1:-1);
  var past=sorted.filter(e=>e.date<now);
  var future=sorted.filter(e=>e.date>=now);
  var MOIS=['Jan','Fév','Mar','Avr','Mai','Jun','Jul','Aoû','Sep','Oct','Nov','Déc'];
  function card(e,isPast){
    var col=EV_COLORS[e.type]||'var(--i3)';
    var ico=EV_TYPES[e.type]||'📌';
    return '<div style="background:#fff;border-radius:var(--R);border:1px solid var(--w2);padding:.85rem 1rem;margin-bottom:8px;display:flex;gap:14px;align-items:flex-start;box-shadow:var(--s1);'+(isPast?'opacity:.45':'')+'">'
      +'<div style="flex-shrink:0;background:'+col+'18;border-radius:var(--r);padding:.45rem .65rem;text-align:center;border:1px solid '+col+'30;min-width:46px">'
      +'<div style="font-size:1.2rem;font-weight:800;color:'+col+';line-height:1;font-family:var(--fd)">'+e.date.slice(8)+'</div>'
      +'<div style="font-size:.58rem;font-weight:700;color:'+col+';text-transform:uppercase;margin-top:1px">'+MOIS[+e.date.slice(5,7)-1]+'</div>'
      +'</div>'
      +'<div style="flex:1">'
      +'<div style="display:flex;align-items:center;gap:8px;margin-bottom:4px">'
      +'<span style="font-size:1rem">'+ico+'</span>'
      +'<span style="font-size:.86rem;font-weight:700;font-family:var(--fd)">'+e.titre+'</span>'
      +'<span style="font-size:.62rem;font-weight:600;padding:2px 7px;border-radius:7px;background:'+col+'18;color:'+col+'">'+e.type+'</span>'
      +'</div>'
      ...
      +'</div>'
      +'<button class="btn btn-d btn-sm" style="flex-shrink:0;align-self:flex-start" onclick="delEvent('+e.id+')">×</button>'
      +'</div>';
  }
  var html='';
  if(future.length){
    html+='<div style="font-size:.72rem;font-weight:700;color:var(--i3);text-transform:uppercase;letter-spacing:.06em;padding:.25rem 0 .65rem">À venir ('+future.length+')</div>';
    html+=future.map(e=>card(e,false)).join('');
  }
  if(past.length){
    html+='<div style="font-size:.72rem;font-weight:700;color:var(--i3);text-transform:uppercase;letter-spacing:.06em;padding:.75rem 0 .65rem;margin-top:.5rem;border-top:1px solid var(--w2)">Passés ('+past.length+')</div>';
    html+=past.map(e=>card(e,true)).join('');
  }
  $('events-list').innerHTML=html||'<div class="empty"><div class="empty-ico">🎪</div><div class="empty-t">Aucun événement</div><div class="empty-s">Cliquez sur "+ Ajouter" pour saisir un événement.</div></div>';
}
...
function svEvent(){
  ...
  apiPost('/api/evenements',d).then(r=>{if(r.ok){EVTS.push(r.item);EVTS.sort((a,b)=>a.date>b.date?1:-1);cm();renderEvents();toast('Événement ajouté');
  ['ev-ti','ev-d','ev-h','ev-lieu','ev-desc','ev-org'].forEach(i=>{var e=$(i);if(e)e.value='';});}});
}
function delEvent(id){
  if(!confirm('Supprimer cet événement ?'))return;
  apiDel('/api/evenements/'+id).then(d=>{if(d.ok){EVTS=EVTS.filter(e=>e.id!==id);renderEvents();toast('Supprimé');}});
}

/* ── SUIVI PROJETS OPÉRATIONNEL ─────────────────────────────────────────────── */
...
let services  = load('services.json', [{"id":1,"nom":"DGS","titre":"Directeur Général des Services"},{"id":2,"nom":"Service Technique","titre":"Voirie & Travaux"},{"id":3,"nom":"CCAS","titre":"Centre Communal d'Action Sociale"},{"id":4,"nom":"Police Municipale","titre":"Police Municipale"},{"id":5,"nom":"Service Culturel","titre":"Culture & Patrimoine"},{"id":6,"nom":"Service Enfance","titre":"Enfance & Jeunesse"}]);
let projExtra     = load('proj_extra.json', {});
let annonces      = load('annonces.json', []);
let tasks         = load('tasks.json', []);
let signalements  = load('signalements.json', []);
...
function buildStats(){
  ...
  projets.forEach(p=>{
    const s=statuts[p.id]||p.statut||'';
    if(s==='Prioritaire')pr++;
    if(p.annee===2026)n26++;
    if(s.toLowerCase().includes('alis'))re++;
    if(s.toLowerCase().includes('cours'))ec++;
    if(s.toLowerCase().includes('tude'))et++;
  });
  const sn=signalements.filter(s=>s.statut==='Nouveau').length;
  const si=signalements.filter(s=>s.statut==='En cours').length;
  return {total:projets.length,prioritaires:pr,annee2026:n26,realises:re,en_cours:ec,etude:et,
          signalements_total:signalements.length,signalements_nouveaux:sn,signalements_cours:si};
}
...
const server = http.createServer(function(req, res) {
  ...
  if(p==='/api/all') return J(res,{
    ...
    annonces, tasks, signalements, evenements, stats:buildStats()
  });
  ...
  if(p==='/api/annonces'&&m==='POST')return body(req,function(err,d){
    if(err)return J(res,{ok:false},400);
    d.id=nextId(annonces);d.ts=ts();d.date=new Date().toISOString();
    annonces.unshift(d);if(annonces.length>50)annonces=annonces.slice(0,50);
    save('annonces.json',annonces);
    return J(res,{ok:true,item:d});
  });
  if(p.match(/^\/api\/annonces\/\d+$/)&&m==='DELETE'){
    const id=parseInt(p.split('/').pop());
    annonces=annonces.filter(a=>a.id!==id);save('annonces.json',annonces);return J(res,{ok:true});
  }

  // ── TÂCHES ────────────────────────────────────────────────────────────────
  if(p==='/api/tasks'&&m==='POST')return body(req,function(err,d){
    if(err)return J(res,{ok:false},400);
    d.id=nextId(tasks);d.created=ts();d.done=false;
    tasks.push(d);save('tasks.json',tasks);
    return J(res,{ok:true,item:d});
  });
  if(p.match(/^\/api\/tasks\/\d+\/done$/)&&m==='PUT'){
    const id=parseInt(p.split('/')[3]);
    tasks=tasks.map(t=>t.id===id?Object.assign({},t,{done:!t.done}):t);save('tasks.json',tasks);
    return J(res,{ok:true});
  }
  if(p.match(/^\/api\/tasks\/\d+$/)&&m==='DELETE'){
    const id=parseInt(p.split('/').pop());
    tasks=tasks.filter(t=>t.id!==id);save('tasks.json',tasks);return J(res,{ok:true});
  }
  ...
  if(p==='/api/signalements'&&m==='POST')return body(req,function(err,d){
    if(err)return J(res,{ok:false},400);
    d.id=nextId(signalements);d.created=ts();d.date=new Date().toISOString();
    d.statut=d.statut||'Nouveau';d.historique=[{statut:'Nouveau',ts:ts(),auteur:d.signale_par||'Inconnu'}];
    signalements.unshift(d);if(signalements.length>200)signalements=signalements.slice(0,200);
    save('signalements.json',signalements);
    const notif={id:Date.now(),titre:'Signalement : '+d.titre,statut:'Nouveau',ancien:'',ts:ts(),new:true,type:'signalement'};
  ...
  if(p.match(/^\/api\/signalements\/\d+\/statut$/)&&m==='PUT')return body(req,function(err,d){
    ...
    signalements=signalements.map(s=>{
      if(s.id!==id)return s;
      const hist=(s.historique||[]).concat([{statut:d.statut,ts:ts(),auteur:d.auteur||'Élu',commentaire:d.commentaire||''}]);
      return Object.assign({},s,{statut:d.statut,historique:hist,updated:ts()});
    });
    save('signalements.json',signalements);
    return J(res,{ok:true});
  });
  if(p.match(/^\/api\/signalements\/\d+$/)&&m==='DELETE'){
    const id=parseInt(p.split('/').pop());
    signalements=signalements.filter(s=>s.id!==id);save('signalements.json',signalements);return J(res,{ok:true});
  }
  ...
  if(p==='/api/evenements'&&m==='POST')return body(req,function(err,d){
    if(err)return J(res,{ok:false},400);
    d.id=nextId(evenements);d.created=ts();
    evenements.push(d);evenements.sort((a,b)=>a.date>b.date?1:-1);
    save('evenements.json',evenements);
    return J(res,{ok:true,item:d});
  });
