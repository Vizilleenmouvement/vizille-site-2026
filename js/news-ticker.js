// Bandeau d'actualités défilant v3
(function() {
    const NEWS_ITEMS = [
        "📢 Nouveau : Consultez notre galerie Médias avec photos et vidéos de la campagne !",
        "📅 Prochaine réunion publique : consultez les Actualités",
        "✨ Site mis à jour le 3 février 2026 - Bilan enrichi avec photos",
        "🗳️ Élections municipales 2026 - Ensemble pour Vizille !"
    ];

    function createTicker() {
        // Vérifier si déjà fermé (24h)
        const lastClosed = localStorage.getItem('newsTickerClosed');
        if (lastClosed) {
            const hoursSinceClosed = (Date.now() - parseInt(lastClosed)) / (1000 * 60 * 60);
            if (hoursSinceClosed < 24) return;
        }

        // Styles
        const style = document.createElement('style');
        style.textContent = `
            .news-ticker {
                background: linear-gradient(90deg, #1a3a5c 0%, #2c5282 50%, #1a3a5c 100%);
                color: white;
                padding: 10px 0;
                overflow: hidden;
                position: relative;
                font-size: 0.95rem;
                border-bottom: 3px solid #E91E63;
            }
            .news-ticker-track {
                display: flex;
                animation: scroll 25s linear infinite;
                white-space: nowrap;
            }
            .news-ticker-item {
                padding: 0 2rem;
                flex-shrink: 0;
            }
            .news-ticker-item::after {
                content: " ★ ";
                color: #E91E63;
                margin-left: 2rem;
            }
            .news-ticker:hover .news-ticker-track {
                animation-play-state: paused;
            }
            @keyframes scroll {
                0% { transform: translateX(0); }
                100% { transform: translateX(-50%); }
            }
            .news-ticker-close {
                position: absolute;
                right: 10px;
                top: 50%;
                transform: translateY(-50%);
                background: rgba(255,255,255,0.2);
                border: none;
                color: white;
                cursor: pointer;
                padding: 4px 10px;
                border-radius: 4px;
                font-size: 1rem;
                z-index: 10;
            }
            .news-ticker-close:hover {
                background: rgba(255,255,255,0.4);
            }
        `;
        document.head.appendChild(style);

        // HTML
        const ticker = document.createElement('div');
        ticker.className = 'news-ticker';
        const items = NEWS_ITEMS.map(item => `<span class="news-ticker-item">${item}</span>`).join('');
        ticker.innerHTML = `
            <div class="news-ticker-track">${items}${items}</div>
            <button class="news-ticker-close" title="Fermer">✕</button>
        `;

        // Fermer
        ticker.querySelector('.news-ticker-close').onclick = function() {
            ticker.remove();
            localStorage.setItem('newsTickerClosed', Date.now().toString());
        };

        // Insérer après le header
        const header = document.querySelector('header');
        if (header) {
            header.insertAdjacentElement('afterend', ticker);
        }
    }

    // Exécuter au chargement
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', createTicker);
    } else {
        createTicker();
    }
})();
